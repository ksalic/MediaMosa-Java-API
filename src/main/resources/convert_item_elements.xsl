...
<xsl:output indent="yes" method="xml"/>
	...
	<xsl:template match="item">
     <!-- convert to still -->
		<xsl:if test="output and content_type and stills">
			<xsl:element name="still">
				<xsl:apply-templates select="@*|node()"/>
			</xsl:element>
		</xsl:if>
		<!-- convert to link -->
    ...
    <!-- convert to still_detail -->
		<xsl:if test="still_id">
			<xsl:element name="still_detail">
				<xsl:apply-templates select="@*|node()"/>
			</xsl:element>
		</xsl:if>
    ...
    <xsl:if test="count and mediafile_id and filesize and filename">
			<xsl:element name="stats_popularstreams">
				<xsl:apply-templates select="@*|node()"/>
			</xsl:element>
		</xsl:if>
